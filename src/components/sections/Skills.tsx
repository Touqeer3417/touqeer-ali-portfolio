            ...
            {skillGroups.map((group, index) => (
              <FadeIn key={group.title} delay={index * 0.06}>
                <div className="h-full rounded-[28px] border border-[var(--line)] bg-[var(--panel)] p-6 sm:p-7">
                  <div className="mb-6 font-mono text-[10px] uppercase tracking-[0.18em] text-[var(--accent)]">
                    {group.title}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2.5">
                    {group.items.map((item) => (
                      <span
                        key={item}
                        className="rounded-full border border-[var(--line)] bg-[var(--surface)] px-3.5 py-2 text-sm text-[var(--foreground)]"
                      >
                        {item}
                      </span>
                    ))}
